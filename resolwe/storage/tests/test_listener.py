...
class ListenerTest(TransactionTestCase):
    ...
    The handlers open their own database transactions, so the tests can not
    run inside the transaction of a ``TestCase``. The commits would nudge the
    manager, which is not running, so its automatic calls are disabled. The
    anonymous user exists: the flush between the tests recreates it.
    """
